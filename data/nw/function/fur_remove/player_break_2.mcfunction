...
execute if entity @s[gamemode=!creative] unless items entity @s weapon.mainhand #hoes run scoreboard players set #check_tool nw 1
execute unless score #disable_noti nw matches 1 if score #check_tool nw matches 1 run tellraw @s [{"text": "[","color": "white"},{"text": "NyaaWorks","color": "#22aaff"},{"text": "]","color": "white"},{"text": " 使用任意 锄头 取下家具","color": "gray"}]
